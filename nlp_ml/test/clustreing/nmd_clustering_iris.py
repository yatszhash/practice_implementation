...
train_X, test_X, train_Y, test_Y = cross_validation.train_test_split(X, Y, test_size=0.2, random_state=random_state)
...
estimator = GaussianMixture(n_components=n_classes, covariance_type=cov_type,
...
import seq_labeling.clustering.ClusteringWithNMD as GMMCluster

import importlib

importlib.reload(GMMCluster)

clf = GMMCluster.ClusteringWithNMD()

clf.fit(train_X, num_clusters=n_classes, random_state=10)
